...
import { Flex, Center, Image, Tab, TabList, Tabs, TabPanels, TabPanel } from "@chakra-ui/react";
...
export default function Campaign({campaign}){
    return (
        <>
            <Head>
                <title>{campaign.title}｜キャンペーン情報｜サムライエナジー</title>
                <meta property="og:type" content="article" />
                <meta property="og:title" content={`${campaign.title}｜キャンペーン情報｜サムライエナジー`} />
                <meta property="og:site_name" content={`${campaign.title}｜キャンペーン情報｜サムライエナジー`} />
                <meta name="twitter:title" content={`${campaign.title}｜キャンペーン情報｜サムライエナジー`} />
            </Head>
            ...
            >
                <Flex className={styles.topicPath}>
                    ...
                    <Link href='/campaign' passHref>
                        <motion.a
                            whileHover={{color: '#333333'}}>CAMPAIGN</motion.a>
                    </Link>
                    <p>＞</p>
                    <p>{campaign.title}</p>
                </Flex>
                    ...
                    <div className={styles.wrap}>
                        ...
                        <div className={styles.container}>
                            <Center className={styles.mv}>
                                <Image src={campaign.Media.url} alt={campaign.title} width='100%' height='auto'/>
                            </Center>
...
export async function getStaticPaths (){
    ...
    const data = await client.get({ endpoint: 'campaign' })
    const paths = data.contents.map((content) => `/campaign/${content.id}`)
...
export async function getStaticProps({params, preview = false}){
    const id = params.id;
    const campaignsData = await client.get({endpoint: 'campaign', contentId: id})
